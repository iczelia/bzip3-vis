...
static double GLOB_COST = 0;
static unsigned GLOB_BYTES_WRT = 0;
static unsigned GLOB_BYTES_TOT = 0;
...
static void encode_bytes(state * s, u8 * buf, s32 size) {
    ...
    for (s32 i = 0; i < size; i++) {
        ...
        while (ctx < 256) {
            ...
            cost += c & 128 ? -log2((ssep * 3 + p) / 262144.0) : -log2(1.0 - (ssep * 3 + p) / 262144.0);
...
BZIP3_API s32 bz3_encode_block(struct bz3_state * state, u8 * buffer, s32 data_size) {
    ...
    GLOB_BYTES_TOT = data_size + overhead * 4 + 1;
    return 0;
}
...
void compute_coding_costs(const char * test_str, float *out_costs) {
    struct bz3_state * state = bz3_new(MiB(1));
    char * copy = malloc(bz3_bound(strlen(test_str)));
    memcpy(copy, test_str, strlen(test_str));
    bz3_encode_block(state, (u8 *)copy, strlen(copy));
    printf("Total bytes (without overhead): %u\n", GLOB_BYTES_WRT);
    printf("Total bytes (with overhead): %u\n", GLOB_BYTES_TOT);
    double prev = 0.0;
    double * cost_tracker = malloc(sizeof(double) * (strlen(test_str) + 1));
    memset(cost_tracker, 0, sizeof(double) * (strlen(test_str) + 1));
    for (int i = 1; i <= strlen(test_str); i++) {
        memset(copy, 0, bz3_bound(strlen(test_str)));
        memcpy(copy, test_str, i);
        bz3_encode_block(state, (u8 *)copy, i);
        cost_tracker[i] = GLOB_COST - prev;  prev = GLOB_COST;
    }
    // approximate mitigation to negative costs.
    int any_fixed = 1;
    while (any_fixed) {
        any_fixed = 0;
        for (int i = 0; i <= strlen(test_str); i++) {
            if (cost_tracker[i] < 0) {
                // cost_tracker[i] must become between 0.0 and 0.1 bits:
                double to_transfer = floor(-cost_tracker[i] * 10) / 10.0;
                cost_tracker[i] = -(cost_tracker[i] + to_transfer);
                cost_tracker[i - 1] -= to_transfer;
                any_fixed = 1;
            }
        }
    }
    for (int i = 0; i <= strlen(test_str); i++) {
        out_costs[i] = (float)cost_tracker[i + 1];
    }
    bz3_free(state);
    free(copy);
}
